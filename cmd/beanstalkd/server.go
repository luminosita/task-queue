...
import (
	...
	"github.com/mnikita/task-queue/pkg/workers"
)
import "github.com/mnikita/task-queue/pkg/consumer"

func main() {
	bConn, err := beanstalk.DialTimeout("tcp", "127.0.0.1:11300", beanstalk.DefaultDialTimeout)

	if err != nil {
		panic(err)
	}

	w := workers.NewWorker(nil)
	c := consumer.NewConsumer(bConn, nil, w)
